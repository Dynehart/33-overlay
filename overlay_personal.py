...
from locations import purple_4k, magenta, yellow
...
def callback():
    ...
    if joy.LeftTrigger > 0:
        ...
        chests = data[version]['chests']
        elites = data[version]['elite']
        for coord in chests:
            result.append(
                FlDrawRect(Vector2D(coord[0]-size, coord[1]-size), size*2, size*2, color, color, 0)
            )
